					...
					<div class="d-inline-block align-items-center">
						<nav>
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">edit employee attendance</li>
							</ol>
						</nav>
					</div>
				</div>
            ...
            <tr id="div{{$value->id}}" class="text-center">
                ...
                <td colspan="3">
                    <div class="switch-toggle switch-3 switch-candy text-center">
                        <input {{$value->attend_status == 'present' ? 'checked' : ''}} name="attend_status{{$key}}" value="present" type="radio" id="present{{$key}}" checked>
                        <label for="present{{$key}}">present</label>

                        <input {{$value->attend_status == 'leave' ? 'checked' : ''}} name="attend_status{{$key}}" value="leave" type="radio" id="leave{{$key}}">
                        <label for="leave{{$key}}">leave</label>

                        <input {{$value->attend_status == 'absent' ? 'checked' : ''}} name="attend_status{{$key}}" value="absent" type="radio" id="absent{{$key}}">
