...
class RegistrationFeeController extends Controller
{
    public function index()
    {
        ...
        return view('backend.student.reg_fee.index_reg_fee', $data);
    }
    public function student_reg_fee_classwise_get(Request $request)
    {
    ...
    public function student_reg_fee_payslip(Request $request)
    {
        $student_id = $request->student_id;
        $class_id = $request->class_id;
        $all['data'] = AssignStudent::with(['student', 'discount'])->where('student_id', $student_id)->where('class_id', $class_id)->first();

        $pdf = PDF::loadView('backend.student.reg_fee.show_reg_fee_pdf', $all);
        $pdf->setPaper('a4', 'potrait');
        $name = $all['data']['student']['name'];
        return $pdf->stream("$name reg_fee-details.pdf");
    }
}
